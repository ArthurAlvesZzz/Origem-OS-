        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left text-sm text-zinc-300">
            <thead className="text-xs uppercase tracking-wider bg-zinc-950/50 text-zinc-500 border-b border-zinc-800/80">
              <tr>
                <th className="px-6 py-4 font-semibold">Data / Vcto</th>
            ...
            <tbody className="divide-y divide-zinc-800/50">
              {payables.map((t) => (
                <tr key={t.id} className="hover:bg-zinc-900/50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right font-medium font-mono text-red-500">- R$ {Math.abs(t.amount).toFixed(2)}</td>
                ...
                <tr>
                  <td colSpan={6} className="py-12">
                    <EmptyState 
                      ...
                      description="Todos os compromissos de pagamento estão em dia ou não há registros no período."
                    />
                  </td>
                </tr>
              )}
