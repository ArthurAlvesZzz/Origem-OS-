    ...
    <div className="bg-zinc-950 border border-zinc-800/80 shadow-inner rounded-xl p-6">
      ...
      <div className="space-y-4 text-sm">
        <div className="flex justify-between text-zinc-400">
          ...
          <span className="font-mono text-zinc-300">R$ {subtotal.toFixed(2)}</span>
        </div>
        
        {totalDiscount > 0 && (
          <div className="flex justify-between text-emerald-500">
            <span>(+) Descontos Aplicados</span>
            <span className="font-mono font-medium">- R$ {totalDiscount.toFixed(2)}</span>
          </div>
        )}
      ...
      <div className="mt-6 pt-5 border-t border-zinc-800/80 bg-zinc-900/50 -mx-6 -mb-6 p-6 rounded-b-xl flex items-center justify-between">
        <div>
          ...
          <div className="text-xs text-zinc-400">Lucro Estimado: <span className="font-medium font-mono text-zinc-300">R$ {margin.toFixed(2)}</span></div>
        </div>
