...
check(jwtSecret.length >= 16, 'JWT_SECRET must be at least 16 characters long.', isProduction);

// 2. DATABASE_URL
const dbUrl = process.env.DATABASE_URL;
if (!dbUrl) {
    check(false, 'DATABASE_URL is missing. The system will rely on mock data or fail for DB-required operations.', isProduction);
}
...
if (!paymentsKey) {
    check(false, 'PAYMENTS_ENCRYPTION_KEY is missing. Payment webhooks validation may be degraded.', isProduction);
} else {
    check(paymentsKey.length >= 16, 'PAYMENTS_ENCRYPTION_KEY must be at least 16 characters long.', isProduction);
}
...
if (mpCount > 0 && mpCount < 3) {
    check(false, 'Mercado Pago variables (MP_CLIENT_ID, MP_CLIENT_SECRET, MP_WEBHOOK_SECRET) are partially configured.', isProduction);
} else if (mpCount === 0) {
...
if (!process.env.PUBLIC_APP_URL && !process.env.APP_PUBLIC_URL) {
    check(false, 'PUBLIC_APP_URL or APP_PUBLIC_URL is missing. Webhooks and internal links may fail.', isProduction);
}
